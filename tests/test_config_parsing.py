"""Tests for config parsing: IPv6 endpoints, FwMark hex, AllowedIPs separators,
Table/SaveConfig, and round-trip fidelity."""
...
class TestIPv6EndpointConfig:
    ...
    def test_roundtrip_ipv6_endpoint(self):
        config = WireguardConfig.from_wgconfig(StringIO(IPV6_CONFIG))
        output = config.to_wgconfig()
        assert "[2607:5300:60:6b0::c05f:543]:2468" in output

    def test_reparsed_ipv6_endpoint(self):
        config = WireguardConfig.from_wgconfig(StringIO(IPV6_CONFIG))
        output = config.to_wgconfig()
...
class TestPeerFromDict:
    def test_ipv6_endpoint_from_dict(self):
        d = {
...
class TestMultipleInterfaceSections:
    def test_raises_on_duplicate_interface(self):
        bad = "[Interface]\nPrivateKey = DnLEmfJzVoCRJYXzdSXIhTqnjygnhh6O+I3ErMS6OUg=\n\n[Interface]\nListenPort = 51820\n"
