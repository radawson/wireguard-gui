"""Tests for CLI command parsing and logic (set, addconf, show argument parsing)."""

import pytest

from wireguard_tools.cli import _parse_set_args, SHOW_FIELDS
...
class TestParseSetArgs:
    ...
    def test_peer_ipv6_endpoint(self):
        key = str(WireguardKey.generate())
        _, _, peers = _parse_set_args(
    ...
    def test_peer_persistent_keepalive(self):
        key = str(WireguardKey.generate())
        _, _, peers = _parse_set_args(
    ...
    def test_peer_persistent_keepalive_off(self):
        key = str(WireguardKey.generate())
        _, _, peers = _parse_set_args(
    ...
    def test_peer_allowed_ips_replace(self):
        key = str(WireguardKey.generate())
        _, _, peers = _parse_set_args(
    ...
    def test_peer_allowed_ips_empty_clears(self):
        key = str(WireguardKey.generate())
        _, _, peers = _parse_set_args(
            ["wg0", "peer", key, "allowed-ips", ""]
        )
        assert peers[0]["allowed_ips"] == []
        assert peers[0]["replace_allowed_ips"] is True

    def test_peer_allowed_ips_incremental(self):
        key = str(WireguardKey.generate())
        _, _, peers = _parse_set_args(
            ["wg0", "peer", key, "allowed-ips", "+10.0.0.0/24,-10.1.0.0/24"]
        )
    ...
    def test_multiple_peers(self):
        ...
        _, _, peers = _parse_set_args([
            "wg0",
            "peer", k1, "allowed-ips", "10.0.0.0/24",
            "peer", k2, "allowed-ips", "10.1.0.0/24",
        ])
        assert len(peers) == 2

    def test_interface_params_and_peers_combined(self):
        key = str(WireguardKey.generate())
        iface, params, peers = _parse_set_args([
            "wg0", "listen-port", "51820",
            "peer", key, "endpoint", "10.0.0.1:51820",
        ])
        assert iface == "wg0"
        assert params["listen_port"] == 51820
        assert len(peers) == 1
    ...
    def test_remove_without_peer_raises(self):
        with pytest.raises(ValueError, match="must follow"):
            _parse_set_args(["wg0", "remove"])

    def test_endpoint_without_peer_raises(self):
        with pytest.raises(ValueError, match="must follow"):
