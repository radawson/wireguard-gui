...
def _show_pretty(
    ...
    """Pretty-print output for human consumption."""
...
def _show_dump(
    ...
    """Tab-separated dump output for scripting."""
...
def _show_field(
    ...
    """Print a single field per the wg show <iface> <field> syntax."""
    pfx = f"{device.interface}\t" if prefix_iface else ""
...
def _parse_set_args(argv: list[str]) -> tuple[str, dict[str, Any], list[dict[str, Any]]]:
    """Parse the wg set argument syntax into (interface, iface_params, peers).

    Syntax:
        wg set <interface> [listen-port <port>] [fwmark <fwmark>]
            [private-key <file>]
            [peer <key> [remove] [preshared-key <file>] [endpoint <ep>]
...
def main() -> int:
    ...
    show_parser.add_argument(
        "field", nargs="?", choices=SHOW_FIELDS, default=None,
    )
